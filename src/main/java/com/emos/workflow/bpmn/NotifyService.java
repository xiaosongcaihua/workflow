...
//        String uuid = variables.get(DeleteProcessByIdForm.ALLATORIxDEMO("/43%")).toString();  // uuid
//        String callbackUrl = variables.get(DeleteProcessByIdForm.ALLATORIxDEMO("4(-")).toString(); // 回调地址
//        String status = variables.get(DeleteProcessByIdForm.ALLATORIxDEMO("($)465")).toString();  // 状态
//
//        // 查询会议信息
//        HashMap<String, Object> meeting = meetingService.searchMeetingByUUID(uuid);
//        String date = meeting.get(DeleteProcessByIdForm.ALLATORIxDEMO("> .$")).toString();      // 日期
//        String endTime = meeting.get(DeleteProcessByIdForm.ALLATORIxDEMO("2. (5")).toString();   // 结束时间
//        String startTime = meeting.get(DeleteProcessByIdForm.ALLATORIxDEMO("$4%")).toString();   // 开始时间
//
//        if ("已通过".equals(status)) {
//            // 更新状态为进行中
//            meetingService.updateMeetingStatus(uuid);
//
//            // 预定会议室的定时任务
...
//            DateTime remindTime = DateUtil.parse(date + " " + endTime, "yyyy-MM-dd HH:mm")
...
//            endStatusMap.put("status", 4);  // 结束状态码
...
//            startStatusMap.put("status", 5);  // 进行中状态码
